                                     ...
                                     const string& filename_to_save)
{
    ...
    //Iterate through all samples row by row and output value between 0 and 1
    for( i = 0; i < nsamples_all; i++ )
    {
        Mat sample = data.row(i);

        float r = model->predict( sample );
		//rdelta -> reference like ASCII Value
...
build_mlp_classifier( const string& data_filename,
                      ...
                      const string& filename_to_load )
{
	//Class count is changed to 26 to represent the Alphabets
    const int class_count = 26;
    Mat data;
    Mat responses;
    ...
    int nsamples_all = data.rows;
	//80% of Samples is used to train
    int ntrain_samples = (int)(nsamples_all*0.8);
    ...
    else
    {
		...
		//For all Trained Samples we associated data with labels
        ...
        // We train classifier, here data.cols=256 which represent no of features
		...
		//Set Activation function SIGMOID_SYM
    ...
    test_and_save_classifier(model, data, responses, ntrain_samples, 'A', filename_to_save); ///-- file used to load into camshift demo (XML, text file)
    return true;
}
...
{
    string filename_to_save = "input_data.txt";
    string filename_to_load = "";
    string data_filename;
    int method = 2;

    cv::CommandLineParser parser(argc, argv, "{data|./letter.txt|}{save||}{load||}{boost||}"
    ...
    if (parser.has("boost"))
        method = 1;
	//Method 2 is used to train the MLP
